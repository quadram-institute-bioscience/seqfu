...
use BioX::Seq::Stream;
use File::Basename;
$Local::PE::VERSION = '0.1.00';
#ABSTRACT: A Paired-End FASTQ files reader, based on Local::Reader.
...
In addition to this, every CPAN release is tested by the L<CPAN testers grid|http://matrix.cpantesters.org/?dist=Local-Reader>.
...
Initialize a new Local::Reader object passing 'B<filename>' argument for the first pairend,
and optionally 'B<rev>' for the second (otherwise can be guessed substituting 'R1' with 'R2' and
'_1.' with '_2.')

  my $pairend = Local::Reader->new({
      filename => "$file_R1",
      rev      => "$file_R2"
  });
...
sub new {
    ...
    if ($self->{interleaved}) {
      # Decode interleaved
      if ($self->{stdin}) {
        $self->{R1} = BioX::Seq::Stream->new();
      } else {
        $self->{R1} = BioX::Seq::Stream->new("$self->{filename}");
      }
    } else {
      # Decode PE
      if ( ! defined $self->{rev} ) {
      ...
      $self->{R1}  = BioX::Seq::Stream->new("$self->{filename}");
      $self->{R2}  = BioX::Seq::Stream->new("$self->{rev}")
